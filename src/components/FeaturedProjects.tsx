            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12">
                {projects.map((p, i) => (
                    <div key={i} className="group">
                        ...
                        <h3 className="text-2xl font-bold mb-2 text-text-main">{p.title}</h3>
                        <p className="text-sm text-text-muted mb-4 line-clamp-2">{p.description}</p>

                        {/* Tech Stack Badges */}
                        <div className="flex flex-wrap gap-2 mb-6">
                            {p.techs.map((tech, j) => (
                                <span key={j} className="text-[10px] font-bold tracking-wider uppercase px-2.5 py-1 rounded bg-primary/10 text-primary border border-primary/20">
                                    {tech}
                                </span>
                            ))}
                        </div>
